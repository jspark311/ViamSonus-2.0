...
* 0.1.6:      Fixed parsing for a recent change in Digikey's HTML. Another ugly part-specific hack.
*
*
*/
...
* Runtime constants                                                                                 *
****************************************************************************************************/
define('DK_P_NUM',      'Digikey_Num');
define('MANU_P_NUM',    'Manu_Num');
define('DK_BASE_URL',   'http://www.digikey.com/product-detail/en/');
define('CACHE_FILE',    '.bom_parse_cache');
define('LOG_FILE',      '.bom_parse_log');
define('UA_STR_FOR_DK', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.77 Safari/537.36');
...
function cURL($url, $post = false){
    ...
    curl_setopt($ch, CURLOPT_COOKIEJAR,      ".bom_parse_cookies");
    curl_setopt($ch, CURLOPT_COOKIEFILE,     ".bom_parse_cookies");
    curl_setopt($ch, CURLOPT_COOKIESESSION,  TRUE);
    
    curl_setopt($ch, CURLOPT_USERAGENT,      UA_STR_FOR_DK);
...
function parse_html_into_array($html) {
  ...
  foreach ($rows as $row) {
    ...
    if (count($fields) >= 2) {
      ...
      $return_value[] = array(trim($fields[0]), trim($fields[1]));
    }
  }
  return $return_value;
}
...
function parse_price_for_part($manu_part, $dk_part) {
  ...
  else if ($dk_part == 'LME49726MY/NOPBCT-ND') {
    $url = 'https://www.digikey.com/product-detail/en/LME49726MY%2FNOPB/LME49726MY%2FNOPBCT-ND';
  }
